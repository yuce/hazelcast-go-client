...
type HeartbeatService struct {
	...
	urgentRequestCh chan<- invocation.Invocation
	doneCh          chan struct{}
	logger          logger.Logger
	state           int32
}

func NewHeartbeatService(cm *ConnectionManager, f *ConnectionInvocationFactory, rCh chan<- invocation.Invocation, logger logger.Logger) *HeartbeatService {
...
func (hs *HeartbeatService) sendHeartbeat(conn *Connection, timeout, interval time.Duration) {
	...
	inv := hs.invFactory.NewConnectionBoundInvocation(request, conn, nil)
	hs.urgentRequestCh <- inv
}
