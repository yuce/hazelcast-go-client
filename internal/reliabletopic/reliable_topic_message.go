...
type Message struct {
	publishTime      int64
	publisherAddress *hazelcast.Address
	payload          serialization.Data
}

func NewMessage(payload serialization.Data, publisherAddr *hazelcast.Address) *Message {
	return &Message{
...
func (r *Message) PublisherAddress() *hazelcast.Address {
...
func (r *Message) ReadData(input serialization.DataInput) (err error) {
	...
	if addr, ok := addrObj.(*hazelcast.Address); ok {
