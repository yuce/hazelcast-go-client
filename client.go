...
func (c *Client) clusterConnected(_ event.Event) {
	//atomic.StoreInt32(&c.lifecycleState, int32(LifecycleStateConnected))
}

func (c *Client) clusterDisconnected(_ event.Event) {
	c.logger.Trace(func() string {
		return "hazelcast.Client.clusterDisconnected"
	})
	go func() {
		c.restartMu.Lock()
		defer c.restartMu.Unlock()
		if atomic.LoadInt32(&c.state) != ready && atomic.LoadInt32(&c.lifecycleState) != int32(LifecycleStateConnected) {
			return
		}
		atomic.StoreInt32(&c.lifecycleState, int32(LifecycleStateDisconnected))
		c.logger.Trace(func() string {
...
func (c *Client) restart(ctx context.Context) {
	...
	if err := c.connectionManager.Restart(ctx); err != nil {
