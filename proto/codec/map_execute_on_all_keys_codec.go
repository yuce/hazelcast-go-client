...
const (
	...
	MapExecuteOnAllKeysCodecRequestInitialFrameSize = proto2.PartitionIDOffset + proto2.IntSizeInBytes
)

// Applies the user defined EntryProcessor to the all entries in the map.Returns the results mapped by each key in the map.

func EncodeMapExecuteOnAllKeysRequest(name string, entryProcessor *iserialization.Data) *proto2.ClientMessage {
	clientMessage := proto2.NewClientMessageForEncode()
	clientMessage.SetRetryable(false)

	initialFrame := proto2.NewFrameWith(make([]byte, MapExecuteOnAllKeysCodecRequestInitialFrameSize), proto2.UnfragmentedMessage)
...
func DecodeMapExecuteOnAllKeysResponse(clientMessage *proto2.ClientMessage) []proto2.Pair {
