...
const (
	...
	MapFetchKeysCodecRequestBatchOffset      = proto2.PartitionIDOffset + proto2.IntSizeInBytes
	MapFetchKeysCodecRequestInitialFrameSize = MapFetchKeysCodecRequestBatchOffset + proto2.IntSizeInBytes
)

// Fetches specified number of keys from the specified partition starting from specified table index.

func EncodeMapFetchKeysRequest(name string, iterationPointers []proto2.Pair, batch int32) *proto2.ClientMessage {
	clientMessage := proto2.NewClientMessageForEncode()
	clientMessage.SetRetryable(true)

	initialFrame := proto2.NewFrameWith(make([]byte, MapFetchKeysCodecRequestInitialFrameSize), proto2.UnfragmentedMessage)
...
func DecodeMapFetchKeysResponse(clientMessage *proto2.ClientMessage) (iterationPointers []proto2.Pair, keys []*iserialization.Data) {
