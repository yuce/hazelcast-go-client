...
func (cdw *ClassDefinitionWriter) WritePortable(fieldName string, portable Portable) error {
	if portable == nil {
		return hazelcast.NewHazelcastSerializationError("cannot write nil portable without explicitly registering class definition", nil)
	}
...
func (cdw *ClassDefinitionWriter) WritePortableArray(fieldName string, portables []Portable) error {
	if portables == nil {
		return hazelcast.NewHazelcastSerializationError("non nil value expected", nil)
	}
	if len(portables) == 0 || portables == nil {
		return hazelcast.NewHazelcastSerializationError("cannot write empty array", nil)
	}
